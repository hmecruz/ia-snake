...
class Safety:
    ...
    def flood_fill(self, grid: Grid, start_pos: tuple[int, int], threshold: Optional[int] = None) -> int:
        ...
        while queue:
            ...
            # Retrieve neighbors
            neighbors = grid.get_neighbours(self.actions, current_pos, None)
            for neighbor_pos, _ in neighbors:
                tile_value = grid.get_tile(neighbor_pos)
                if neighbor_pos not in visited and tile_value != Tiles.STONE:
